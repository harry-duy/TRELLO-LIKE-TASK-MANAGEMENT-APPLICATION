...
const L = {
  vi: {
    title: 'Lưu trữ',
    subtitle: 'Các card đã lưu trữ trong board này',
    ...
    deleteConfirm: 'Xoá vĩnh viễn card này? Hành động này không thể hoàn tác.',
    ...
    loading: 'Đang tải...',
    retry: 'Thử lại',
    loadFail: 'Không thể tải dữ liệu.',
  },
  en: {
    title: 'Archive',
...
export default function ArchivePanel({ boardId, lang = 'vi', onClose }) {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (error) => toast.error(error?.message || l.deleteFail),
  });
      ...
      >
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 22 }}>Archive</span>
            <div>
              <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: 'white' }}>{l.title}</h2>
              <p style={{ margin: '2px 0 0', fontSize: 12, color: 'rgba(255,255,255,.45)' }}>{l.subtitle}</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div style={{ maxHeight: 400, overflowY: 'auto', padding: '0 20px 20px' }}>
          {isLoading ? (
            ...
            <div style={{ textAlign: 'center', padding: '24px 16px' }}>
              <p style={{ margin: 0, fontSize: 13, color: '#f87171' }}>{l.loadFail}</p>
              <button
                type="button"
                onClick={() => refetch()}
                style={{
